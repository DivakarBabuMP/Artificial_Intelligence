...
class Puzzle:
    ...
    def dfs(self):
        """Depth-First Search algorithm to find the solution to the 8-puzzle problem."""
        start_node = Node(state=self.start, parent=None, action=None)  # Create the root node
        frontier = [start_node]  # Frontier stack for DFS
        visited = set()  # Set to store visited states

        while frontier:
            node = frontier.pop()  # Get the node from the top of the stack
            ...
            for action, state in self.neighbors(node.state):
                if tuple(map(tuple, state[0])) not in visited:
                    ...
                    frontier.append(child_node)  # Add the child to the frontier stack
...
if __name__ == "__main__":
    ...
    solution = puzzle.dfs()  # Solve the puzzle using DFS
